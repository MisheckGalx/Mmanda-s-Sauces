...
function App() {
  ...
  const products = [
    ...
    { 
      ...
      rating: 4.9, 
      reviews: 156, 
      description: 'Smooth and creamy with a perfect kick.', 
    ...
    { 
      ...
      rating: 4.7, 
      reviews: 89, 
      description: 'Bold garlic meets fiery heat.', 
    ...
    { 
      ...
      rating: 4.6, 
      reviews: 98, 
      description: 'Honey-sweetened with a spicy finish.', 
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* NAVBAR - Bright White with Red Accents */}
      <nav className={`fixed w-full top-0 z-50 transition-all duration-300 ${scrolled ? 'bg-white shadow-xl' : 'bg-white/95'}`}>
      ...
      {/* HERO - Bright Red Background */}
      <section id="home" className="relative min-h-screen flex items-center justify-center overflow-hidden bg-gradient-to-br from-red-600 via-red-500 to-orange-500">
        ...
        <div className="relative z-10 text-center px-4 max-w-6xl mx-auto pt-20">
          <div className="inline-flex items-center space-x-2 px-6 py-3 bg-white/20 backdrop-blur-xl rounded-full mb-8 border-2 border-white/40">
            <Sparkles className="text-yellow-300" size={20} />
            <span className="text-sm font-bold text-white">🇿🇦 Handcrafted in South Africa</span>
          </div>
          ...
          <p className="text-2xl md:text-3xl text-white mb-8 max-w-2xl mx-auto font-bold drop-shadow-lg">
            Premium Hot Sauces • 100% Natural • Zero Preservatives
          </p>
          ...
          <div className="grid grid-cols-3 gap-8 max-w-3xl mx-auto">
            {[
              { icon: '⭐', value: '4.8/5', label: 'RATED' },
              { icon: '🌿', value: '100%', label: 'NATURAL' },
              { icon: '🔥', value: '1000+', label: 'CUSTOMERS' }
            ].map((stat, idx) => (
              <div key={idx} className="p-6 bg-white/20 backdrop-blur-xl rounded-3xl border-2 border-white/40">
      ...
      {/* ABOUT - White Background with Colorful Accents */}
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <div className="relative">
              ...
              />
              <div className="hidden w-full h-96 bg-gradient-to-br from-red-500 to-orange-500 rounded-3xl items-center justify-center text-9xl">
                🌶️
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-8">
                {[
                  { icon: '🌿', text: '100% Natural', color: 'bg-green-100 border-green-500' },
                  { icon: '🔥', text: 'Bold Flavor', color: 'bg-red-100 border-red-500' },
                  { icon: '🏆', text: 'Award Winning', color: 'bg-yellow-100 border-yellow-500' },
                  { icon: '🇿🇦', text: 'Proudly SA', color: 'bg-orange-100 border-orange-500' }
                ].map((item, idx) => (
                  <div key={idx} className={`flex items-center space-x-3 p-4 ${item.color} border-2 rounded-2xl shadow-md hover:shadow-xl transition`}>
                    <span className="text-4xl">{item.icon}</span>
                    <span className="font-black text-gray-900">{item.text}</span>
                  </div>
                ))}
              </div>
      ...
      {/* PRODUCTS - White Background with Colorful Cards */}
      <section id="products" className="py-32 px-4 bg-white">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
            {products.map((p) => (
              <div key={p.id} className="group relative bg-white rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-500 overflow-hidden border-4 border-gray-100 hover:border-red-500">
                
                <div className="absolute top-4 right-4 z-10">
                  <span className={`px-4 py-2 ${p.bgColor} text-white rounded-full text-xs font-black shadow-lg`}>
                    {p.badge}
                  </span>
                </div>
                ...
                <div className="relative h-64 bg-gradient-to-br from-gray-100 to-gray-200 overflow-hidden">
                  ...
                  />
                  <div className="hidden w-full h-full bg-gradient-to-br from-red-400 to-orange-400 items-center justify-center text-8xl">
                    {p.emoji}
                  </div>
                </div>
                ...
                <div className="p-6">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="flex items-center space-x-1 px-3 py-1 bg-yellow-400 rounded-lg">
                      <Star size={14} className="fill-gray-900 text-gray-900" />
                      <span className="text-sm font-black text-gray-900">{p.rating}</span>
                    </div>
                  </div>

                  <p className="text-gray-600 text-sm mb-4 font-medium">{p.description}</p>
                  ...
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <div className="text-xs text-gray-500 font-semibold">{p.reviews} reviews</div>
                  </div>

                  <button
                    onClick={() => addToCart(p)}
                    className={`w-full py-4 bg-gradient-to-r ${p.color} text-white rounded-2xl font-black shadow-lg hover:shadow-2xl transition-all transform hover:scale-105 flex items-center justify-center space-x-2`}
                  >
      ...
      {/* GALLERY - Bright Yellow Background */}
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {[1, 2, 3].map((num) => (
              ...
              >
                ...
                />
                <div className="hidden w-full h-96 bg-gradient-to-br from-red-500 to-orange-500 items-center justify-center text-8xl">
                  🌶️
                </div>
              </div>
            ))}
      ...
      <section id="contact" className="py-32 px-4 bg-red-600">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-16">
            <div className="inline-block px-6 py-3 bg-yellow-400 rounded-full mb-6">
              <span className="text-sm font-black text-gray-900">GET IN TOUCH</span>
            </div>
            <h2 className="text-5xl md:text-6xl font-black mb-4 text-white">
              LET'S CONNECT
            </h2>
            <p className="text-xl text-white font-bold">Questions? Orders? We're here to help!</p>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            ...
            <button
              ...
              className="group p-8 bg-white rounded-3xl shadow-2xl hover:shadow-3xl transition-all transform hover:-translate-y-2 text-center"
            >
              <div className="inline-flex p-5 bg-blue-500 rounded-2xl mb-6 group-hover:scale-110 transition">
            ...
            <button
              ...
              className="group p-8 bg-white rounded-3xl shadow-2xl hover:shadow-3xl transition-all transform hover:-translate-y-2 text-center"
            >
              <div className="inline-flex p-5 bg-gradient-to-r from-purple-500 to-pink-500 rounded-2xl mb-6 group-hover:scale-110 transition">
      ...
      <footer className="bg-gray-900 py-12 px-4">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <p className="text-red-400 text-sm font-bold mb-2">Handcrafted • Authentic • Bold</p>
          <p className="text-gray-500 text-xs font-semibold">© 2026 All rights reserved. Made with 🔥 in South Africa</p>
        </div>
      </footer>
      ...
      <button
        onClick={() => window.open('https://wa.me/27626456655', '_blank')}
        className="fixed bottom-8 right-8 z-40 p-6 bg-green-500 hover:bg-green-600 rounded-full shadow-2xl transition-all transform hover:scale-110 animate-bounce"
