            ...
            <div className="grid md:grid-cols-2 gap-8 max-w-4xl">
              {products.map((p) => (
                <div key={p.id} className="group relative bg-white rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-500 overflow-hidden border-4 border-gray-100 hover:border-black">
                  ...
                  <div className="p-8">
                    <h3 className="text-3xl font-black text-gray-900 mb-6">{p.name}</h3>

                    <div className="flex items-center justify-between mb-6">
                      <div className="text-5xl font-black text-red-600">
                        R{p.price}
                      </div>
                      <div className="text-sm text-gray-500 font-bold">{p.stock} in stock</div>
                    </div>

                    <button
                      onClick={() => addToCart(p)}
                      className="w-full py-5 bg-black hover:bg-gray-800 text-white rounded-2xl font-black text-lg shadow-lg hover:shadow-2xl transition-all transform hover:scale-105 flex items-center justify-center space-x-2"
                    >
